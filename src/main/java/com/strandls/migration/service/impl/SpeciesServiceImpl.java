...
//	migration of field to new structure
	...
	public void migrateField() {
		...
		try {
			List<Field> fieldList = fieldDao.findByLanguageId(123L);
...
//					save the object
					...
					Field fieldFrench = fieldDao.findByConnectionId(field.getId(), 137L);
					fieldHeader = new FieldHeader(null, fieldNew.getId(), fieldFrench.getConcept(),
							(fieldFrench.getCategory() == null) ? fieldFrench.getDescription() : null,
							(fieldFrench.getCategory() == null) ? fieldFrench.getUrlIdentifier() : null,
							fieldFrench.getLanguageId());
					fieldHeaderDao.save(fieldHeader);
...
//				save the obj
				...
				Field fieldFrench = fieldDao.findByConnectionId(field.getId(), 137L);
				fieldHeader = new FieldHeader(null, fieldNew.getId(), fieldFrench.getCategory(),
						fieldFrench.getDescription(), fieldFrench.getUrlIdentifier(), fieldFrench.getLanguageId());
				fieldHeaderDao.save(fieldHeader);
...
//				save obj
				...
				Field fieldFrench = fieldDao.findByConnectionId(field.getId(), 137L);
				fieldHeader = new FieldHeader(null, fieldNew.getId(), fieldFrench.getSubCategory(),
						fieldFrench.getDescription(), fieldFrench.getUrlIdentifier(), fieldFrench.getLanguageId());
				fieldHeaderDao.save(fieldHeader);
	...
	public void migrateSpeciesField() {
		List<SpeciesField> speciesFieldList = sfDao.findAll();
		for (SpeciesField sf : speciesFieldList) {

			Field frenchField = fieldDao.findById(sf.getFieldId());
			if (frenchField.getId() != frenchField.getConnection()) {
				sf.setFieldId(frenchField.getConnection());
				sfDao.update(sf);
			}
